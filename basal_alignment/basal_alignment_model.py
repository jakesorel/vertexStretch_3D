...
def get_E(Y,tissue_params,mesh_props,nc):
    ...
    E_A = tissue_params["kappa_A"]*(mesh_props["A"]-tissue_params["A0"])**2
    E_A = E_A[:7].sum()
    ...
    z_av = jnp.expand_dims(mesh_props["z"].mean(axis=1),axis=1)*jnp.ones_like(mesh_props["z"])
    z_imposed = z_av.copy()
    z_imposed = z_imposed.at[mesh_props["central_tri_index"]].set(mesh_props["z"][mesh_props["central_tri_index"]])
...
def get_A_tot(positive_indices,pos_gamma_mult,Gamma,_mesh_props,minimizer_params):
    mesh_props = deepcopy(_mesh_props)

    A0 = np.ones(nc) * 5.2
    # A0[positive_indices] = A0_pos
    gamma = np.ones(nc) * 0.08
    gamma[positive_indices] *= pos_gamma_mult
    p_notch = len(positive_indices)/7
    print(p_notch)
    gamma[7:] = p_notch * gamma.min() + (1 - p_notch) * gamma.max()
...
Gamma_range = 10.0**(np.array([-200,1]))
A_vals,L_vals = np.zeros((len(Gamma_range),26)),np.zeros((len(Gamma_range),26))
for i, Gamma in enumerate(Gamma_range):
    A_vals[i],L_vals[i] = np.array([get_A_tot(np.nonzero(is_notch)[0],0.5,Gamma,mesh_props,minimizer_params) for is_notch in tqdm(is_notchs)]).T
...
for i in range(len(Gamma_range)):
    sns.lineplot(data=df,x="count",y="A_%d"%i,color=plt.cm.plasma(i/len(Gamma_range)))
...
def get_E_L(Y,L,tissue_params,mesh_props,nc):
    x_dof = Y[:12].reshape(6,2)
    n_dof = Y[12:12+14].reshape(7,2)
    # L = jnp.abs(Y[-1])
    mesh_props["x"] = mesh_props["x"].at[:6].set(x_dof)
    mesh_props["L"] = L
    mesh_props["n"] = mesh_props["n"].at[:7].set(n_dof)*0
    mesh_props = get_geometry(mesh_props,nc)
    E_i = tissue_params["kappa_A"]*(mesh_props["A"]-tissue_params["A0"])**2 + \
        tissue_params["gamma"]*(mesh_props["P"])
    boundary_mask = (mesh_props["tri"]<=7)*(jnp.roll(mesh_props["tri"],-1,axis=1)>7)
    l_boundary = (mesh_props["lt"]*boundary_mask).sum()
    ...
    z_imposed = z_imposed.at[mesh_props["central_tri_index"]].set(mesh_props["z"][mesh_props["central_tri_index"]])
    return E_i[:7].sum() \
        + tissue_params["Gamma"]*((z_imposed-z_av)**2).sum() \
        + tissue_params["gamma_mean"]*l_boundary
...
tissue_params = {"kappa_A": 0.02,
                 ...
                 "Gamma": Gamma}
p_notch = len(positive_indices) / 7
tissue_params["gamma_mean"] = gamma.min() * p_notch + gamma.max() * (1 - p_notch)

n_dof = np.zeros((7,2))
x_dof = mesh_props["x"][:6]
L = 0.1
...
def get_E_L_eq(L,positive_indices,pos_gamma_mult,Gamma,_mesh_props,minimizer_params):
    mesh_props = deepcopy(_mesh_props)
    A0 = np.ones(nc)*5.2
    # A0[positive_indices] = A0_pos
    gamma = np.ones(nc)*0.13
...
L_range = np.logspace(-1,1,10)
# E_L = np.array([get_E_L_eq(L,positive_indices,pos_gamma_mult,0.0,_mesh_props,minimizer_params) for L in tqdm(L_range)])
E_L_1000 = np.array([get_E_L_eq(L,positive_indices,pos_gamma_mult,0.1,_mesh_props,minimizer_params) for L in tqdm(L_range)])
...
L_range = np.linspace(0.1,3.5,10)

A_vals1 = np.array([np.array([get_E_L_eq(L,np.nonzero(is_notch)[0],0.1,0,mesh_props,minimizer_params) for is_notch in tqdm(is_notchs)]) for L in L_range])

mean_A_count = np.array([np.bincount(count,A_vals1[i])/np.bincount(count) for i in range(10)])
