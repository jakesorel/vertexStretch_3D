...
def run_simulation(T_cortical,alpha,A0,p_notch,file_name,seed,index):
    try:
        ...
        T_cortical,alpha,A0,p_notch,file_name,seed = (0.1909090909090909, 0.3, 7.045454545454546, 0.6, 'results', 3)
        ...
        plt.scatter(*sim.t.mesh.mesh_props["R"][:,:2].T,c=is_notch_sets[-1])
        plt.show()
    ...
    fig, ax = plt.subplots()
    ax.plot(np.linspace(0,1,6),np.flip(L_outs))
